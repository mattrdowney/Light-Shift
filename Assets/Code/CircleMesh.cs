...
public static class Circle
{
    ...
    public static Mesh circle_mesh()
    {
        if (circle == null)
        {
            ...
            for (int sector = 0; sector < sectors; ++sector)
            {
                ...
                vertex_list[sector*3 + 0] = Vector3.zero + offset; // All sectors include (0, 0).
                vertex_list[sector*3 + 1] = new Vector3(Mathf.Cos(sweep_start), Mathf.Sin(sweep_start), 0)*radius + offset;
                vertex_list[sector*3 + 2] = new Vector3(Mathf.Cos(sweep_end), Mathf.Sin(sweep_end), 0)*radius + offset;
    ...
    private const float radius = 0.4817366f;
    private static readonly Vector3 offset = (radius - 0.36f)*Vector3.down;
}
